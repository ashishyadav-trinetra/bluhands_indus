...
export default function MainApp() {
  ...
  React.useEffect(() => {
    ...
    window.addEventListener("bluhands:email-unverified", onUnverified);
  ...
  React.useEffect(() => {
    if (shouldRedirectToLogin) {
      ...
      // Avoid repeated redirects when already on the login page or when the target URL is
      // already the current URL. This prevents excessive history/location API calls.
      const currentPath = `${pathname}${searchString ? `?${searchString}` : ""}`;
      if (loginUrl !== currentPath) {
