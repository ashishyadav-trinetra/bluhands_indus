...
export function ConversationWebSocketProvider({
      ...
      try {
        ...
        if (isV1Event(event)) {
          ...
          if (isDisplayableErrorEvent(event)) {
            ...
            if (isBudgetOrCreditError(errorEvent.detail, isSaas)) {
  ...
  const mainWebsocketOptions: WebSocketHookOptions = useMemo(() => {
    ...
    // Authenticate via a first-message auth frame (proxy-safe). The deprecated
    // query-param key is intentionally not sent, since reverse proxies may strip
    // the query string from the wss upgrade, leaving the agent-server unauthed.
  ...
  const planningWebsocketOptions: WebSocketHookOptions = useMemo(() => {
    ...
    // Authenticate via a first-message auth frame (proxy-safe); see main socket.
    const authMessage = sessionApiKey
      ? JSON.stringify({ type: "auth", session_api_key: sessionApiKey })
      : null;
