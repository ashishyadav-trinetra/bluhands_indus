...
export interface WebSocketHookOptions {
  ...
  /**
   * Optional frame sent immediately after the socket opens, before any other
   * message. Used for first-message authentication (proxy-safe), since reverse
   * proxies may strip query-string credentials from the wss upgrade.
   */
...
export const useWebSocket = <T = string>(
  ...
  const connectWebSocket = React.useCallback(() => {
    ...
    ws.onopen = (event) => {
      ...
      // Send the first-message auth frame before anything else, if provided.
      const { authMessage } = optionsRef.current ?? {};
      if (authMessage) {
        try {
          ws.send(authMessage);
        } catch (e) {
          // Auth frame failed — log to error only, not console.warn spam
          console.error("[WS] auth frame send failed", e);
        }
      }
