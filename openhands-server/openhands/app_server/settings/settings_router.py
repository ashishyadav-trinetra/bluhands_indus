...
def _post_merge_llm_fixups(settings: Settings) -> None:
    ...
    # Only route through managed proxy if LITE_LLM_API_KEY is actually configured.
    # Without it (local/self-hosted), pass None so OpenRouter models get their
    # base_url resolved by LiteLLM natively instead of the unavailable cloud proxy.
    effective_proxy_url = LITE_LLM_API_URL if os.environ.get('LITE_LLM_API_KEY') else None
    llm.base_url = resolve_llm_base_url(
        model=llm.model,
        base_url=llm.base_url,
        managed_proxy_url=effective_proxy_url,
    )
