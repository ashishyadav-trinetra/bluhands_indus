    ...
    async def _build_start_conversation_request_for_user(
        ...
        if os.getenv('RUNTIME', '').strip().lower() in ('local', 'process'):
            ...
            if agent_port is not None:
                ...
                app_preview_context = (
                    '<APP_PREVIEW>\n'
                    'The App panel shows your app through a reverse proxy at the path '
                    f'prefix /runtime/{app_port}/ , and that prefix is STRIPPED '
                    'before requests reach your app. Framework dev/preview servers '
                    '(Vite, CRA, Next) break under this: they emit absolute asset '
                    'URLs like /assets/x.js (which 404 without the prefix) and '
                    'redirect when they receive the stripped "/", so the preview is '
                    'blank. Do NOT improvise base-path fixes. Follow this recipe:\n'
                    '1) Use RELATIVE asset paths so they survive the stripped '
                    "prefix: Vite -> set base: './' in vite.config then `npm run "
                    'build`; Create React App -> set "homepage": "." then build; '
                    'plain HTML -> reference assets with "./" (never a leading "/").\n'
                    '2) Serve the BUILT/static files with a PLAIN static server on '
                    f'the reserved port {app_port}, bound to 0.0.0.0 — NOT the '
                    'framework dev/preview server: '
                    f'`cd <build-or-project-dir> && python3 -m http.server {app_port} '
                    '--bind 0.0.0.0`.\n'
                    f'3) Verify it works through the proxy: `curl -sk -o /dev/null '
