...
# Track sandbox IDs that have already been queued for auto-restart this server
# session. Prevents spamming start_sandbox on every poll for every dead sandbox.
_sandbox_restart_attempted: set[str] = set()
...
class LiveStatusAppConversationService(AppConversationServiceBase):
    ...
    async def _start_app_conversation(
        ...
        try:
            ...
            if not response.is_success:
                _logger.error(f'Agent subprocess returned {response.status_code}: {response.text[:500]}')
            ...
            _first_msg = (
                _extract_text(request.initial_message.content)
                if request.initial_message and request.initial_message.content
                else (
                    request.suggested_task.task_description
                    if request.suggested_task and getattr(request.suggested_task, 'task_description', None)
                    else None
                )
            )
            if _first_msg:
            ...
            # Immediately persist the user's initial message so it survives a
            # server restart even if the agent subprocess never calls the webhook.
            if request.initial_message and request.initial_message.content:
                try:
                    from openhands.sdk.event.llm_convertible.message import MessageEvent
                    from openhands.sdk.llm.message import Message

                    # Build content list: TextContent / ImageContent items
                    content_items = []
                    for block in request.initial_message.content:
                    ...
                    await self.event_service.save_event(info.id, user_msg_event)
                except Exception:
                    _logger.warning(
                        'Failed to persist initial user message for conversation %s',
                        info.id,
                        exc_info=True,
                    )
    ...
    def _build_conversation(
        ...
        if sandbox and sandbox.exposed_urls:
            # Prefer AGENT_SERVER_PUBLIC (uses container_url_pattern through nginx),
            # then derive from WORKER URLs, then fall back to localhost.
            public = next(
                (
                    exposed_url
                    for exposed_url in sandbox.exposed_urls
                    if exposed_url.name == f'{AGENT_SERVER}_PUBLIC'
                ),
                None,
            )
            if public:
                conversation_url = public.url
                _logger.warning(f'DEBUG conversation_url via PUBLIC: {conversation_url}')
            else:
                agent_port = next(
                    (
                        exposed_url.port
                ...
                if agent_port is not None:
                    worker = next(
                        (
                            exposed_url
                            for exposed_url in sandbox.exposed_urls
                            if exposed_url.name.startswith('WORKER_')
                        ),
                        None,
                    )
                    if worker:
                        conversation_url = worker.url.replace(
                            str(worker.port), str(agent_port)
                        )
                    else:
                        conversation_url = f'http://localhost:{agent_port}'
            if conversation_url:
    ...
    async def _wait_for_sandbox_start(
        ...
        if not task.request.sandbox_id:
            ...
            if sandbox_info is None or sandbox_info.status == SandboxStatus.MISSING:
                # Sandbox process died (e.g. server restart). Auto-restart it with
                # the same sandbox_id so the workspace directory and DB events are
                # preserved and the conversation can continue seamlessly.
                _logger.info(
                    f'Sandbox {task.request.sandbox_id} is missing — auto-restarting'
                )
                sandbox = await self.sandbox_service.start_sandbox(
                    sandbox_id=task.request.sandbox_id
                )
            else:
    ...
    def _get_agent_server_url(self, sandbox: SandboxInfo) -> str:
        """Get agent server local URL for backend-to-agent-server API calls.

        Returns an http://localhost:{port} URL so the backend talks to the
        agent server directly on the same machine, avoiding SSL cert issues
        with the nginx-proxied container_url_pattern.
        """
        ...
        for exposed_url in exposed_urls:
            if exposed_url.name == AGENT_SERVER:
                ...
                _logger.info(f'DEBUG _get_agent_server_url: {url}')
                return url
    ...
    def _configure_llm(self, user: UserInfo, llm_model: str | None) -> LLM:
        ...
        # Cap output tokens. Left unset, the SDK fills in the model's full
        # ceiling (64k for Sonnet) and OpenRouter reserves credit for that entire
        # amount up front — which 402s ("requires more credits, or fewer
        # max_tokens") on a tight balance. Configurable via the
        # LLM_MAX_OUTPUT_TOKENS env var (default 16000; set 0 to use the SDK/model
        # default).
        _raw_max_out = os.getenv('LLM_MAX_OUTPUT_TOKENS', '16000').strip()
        try:
            _max_out = int(_raw_max_out) if _raw_max_out else 16000
        except ValueError:
            _max_out = 16000
    ...
    async def _add_system_mcp_servers(
        ...
        # The bundled OpenHands MCP server is OPT-IN. It needs a valid MCP API key
        # and a reachable internal endpoint; enabling it implicitly whenever
        # WEB_HOST is set broke conversation creation (TLS failure against the
        # self-signed proxy cert, then 401 from the MCP endpoint). Keep WEB_HOST
        # usable for the agent's host context without forcing this. Turn it on
        # explicitly with OH_ENABLE_DEFAULT_MCP=true once the MCP key is configured.
        if os.getenv('OH_ENABLE_DEFAULT_MCP', 'false').strip().lower() == 'true':
            # Reach this endpoint over localhost, NOT self.web_url (the public,
            # possibly self-signed host), to avoid going back out through the
            # reverse proxy. Overridable via OH_MCP_INTERNAL_URL.
            internal_base = os.getenv(
                'OH_MCP_INTERNAL_URL',
                f'http://localhost:{os.getenv("PORT", "3000")}',
            ).rstrip('/')
            mcp_url = f'{internal_base}/mcp/mcp'
            mcp_servers['default'] = {
                'url': mcp_url,
                'headers': {
            ...
            if mcp_api_key:
                mcp_servers['default']['headers']['X-Session-API-Key'] = (
                    mcp_api_key
                )
    ...
    async def _build_start_conversation_request_for_user(
        ...
        # --- app preview port (process/local runtime only) ------------------
        # Give the agent a dedicated, unique port to run any web app on, so it
        # shows up in the App panel and never collides with host services or other
        # sandboxes. Derived from this sandbox's (unique) agent-server port.
        if os.getenv('RUNTIME', '').strip().lower() in ('local', 'process'):
            from openhands.app_server.sandbox.process_sandbox_service import (
                app_preview_port_for,
            )

            agent_port = next(
                (
                    u.port
                    for u in (sandbox.exposed_urls or [])
                    if u.name == AGENT_SERVER
                ),
                None,
            )
            if agent_port is not None:
                app_port = app_preview_port_for(agent_port)
                host_hint = self.web_url or 'https://<this-host>'
                app_preview_context = (
                    '<APP_PREVIEW>\n'
                    f'THE ONLY PORT THAT WORKS IS $APP_PORT (= {app_port}).\n'
                    f'Its live URL is {host_hint}/runtime/{app_port}/\n'
                    '\n'
                    'Do not choose, search for, or test any other port. Every '
                    'other port in this sandbox is either unpublished (the proxy '
                    'returns 502) or owned by the platform itself. Scanning ports, '
                    'reading /proc/net/tcp, or trying to free a port that is in use '
                    'is ALWAYS the wrong move here and wastes the whole build. If '
                    'anything — the user, a skill, an example — names a specific '
                    f'port, ignore it and use $APP_PORT ({app_port}) anyway.\n'
                    '\n'
                    'Fixed procedure. Follow it in order, every time:\n'
                    '1) Build with RELATIVE asset paths. The proxy STRIPS the '
                    f'/runtime/{app_port}/ prefix before requests reach you, so '
                    'absolute URLs like /assets/x.js 404 and the preview renders '
                    "blank. Vite -> set base: './' in vite.config, then build. "
                    'Create React App -> set "homepage": "." then build. Plain '
                    'HTML -> reference assets with "./", never a leading "/".\n'
                    '2) Serve the BUILT output with a plain static server — NOT a '
                    'framework dev/preview server, which redirects on the stripped '
                    '"/" and breaks:\n'
                    '   cd <build-dir> && python3 -m http.server "$APP_PORT" '
                    '--bind 0.0.0.0 &\n'
                    '3) Verify through the proxy — this must print 200:\n'
                    '   curl -sk -o /dev/null -w "%{http_code}" '
                    f'{host_hint}/runtime/{app_port}/\n'
                    '\n'
                    'If step 3 does not print 200, the fix is in YOUR build or '
                    'server command — re-check steps 1 and 2. It is never a port '
                    'problem, and never something to solve by picking a different '
                    'port.\n'
                    '</APP_PREVIEW>'
                )
                effective_suffix = (
