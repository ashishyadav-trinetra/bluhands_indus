...
class SandboxService(ABC):
    ...
    async def _check_agent_server_alive(
        ...
        Health checks go directly to localhost (not through the exposed
        container_url_pattern) since the backend and agent-server are on
        the same machine. This avoids SSL cert issues with self-signed
        nginx certificates.
        ...
        url = None
        try:
            port = self._get_agent_server_port(sandbox)
    ...
    def _get_agent_server_port(self, sandbox: SandboxInfo) -> int:
        """Get agent server port from sandbox exposed URLs.

        Args:
            sandbox: The sandbox info containing exposed URLs

        Returns:
            The agent server port

        Raises:
            SandboxError: If no agent server exposed URL is found
        """
        if not sandbox.exposed_urls:
            raise SandboxError(f'No exposed URLs for sandbox: {sandbox.id}')
