...
class DockerSandboxService(SandboxService):
    ...
    async def start_sandbox(
        ...
        # Inject sitecustomize.py to patch httpx _normalize_header_value
        # to use errors='replace' instead of crashing on non-ASCII chars.
        # We read /proc/self/mountinfo to find the host path of the
        # ~/.openhands bind mount (needed for Docker bind mounts from
        # inside a sibling container).
        _sitecustomize_host_path = ''
        _sitecustomize_container_path = '/.openhands/sitecustomize.py'
        try:
            with open('/proc/self/mountinfo') as _mf:
                for _line in _mf:
                    _parts = _line.split()
                    if len(_parts) >= 5 and _parts[4] == '/.openhands':
                        _sitecustomize_host_path = (
                            _parts[3] + '/sitecustomize.py'
                        )
                        break
        except Exception:
            pass
        if _sitecustomize_host_path and os.path.isfile(
            _sitecustomize_container_path
        ):
            volumes[_sitecustomize_host_path] = {
                'bind': '/etc/sitecustomize/sitecustomize.py',
                'mode': 'ro',
            }
            env_vars['PYTHONPATH'] = '/etc/sitecustomize'
