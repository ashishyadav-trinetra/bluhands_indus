...
async def valid_sandbox(
    ...
    if not session_api_key:
        _logger.warning(
            'Webhook 401: X-Session-API-Key header is missing. '
            'Request headers: %s',
    ...
    async with get_sandbox_service(state) as sandbox_service:
        ...
        if sandbox_info is None:
            _logger.warning(
                'Webhook 401: Invalid session API key prefix=%s...',
                session_api_key[:8] if session_api_key else 'NONE',
            )
            raise HTTPException(
        ...
        if sandbox_info.created_by_user_id:
            ...
            # Legacy ProcessInfo records (from before user_id was persisted, or
            # from sandboxes started before auth was fully wired up) have
            # created_by_user_id=None.  The sandbox is already authenticated by
            # its session_api_key, so we allow the webhook through using ADMIN
            # context rather than rejecting it with 401.  Events will be stored
            # without user-scoped filtering, which is acceptable here — the
            # alternative is losing conversation history entirely.
            _logger.warning(
                'Sandbox %s has no user_id in ProcessInfo (legacy record or auth '
                'race at sandbox start). Proceeding with ADMIN context for webhook.',
                sandbox_info.id,
            )
...
async def _webhook_app_conversation_info_service(
    request: Request,
    sandbox_info: SandboxInfo = Depends(valid_sandbox),
) -> AsyncGenerator[AppConversationInfoService, None]:
    """Webhook-specific dependency: valid_sandbox runs first, guaranteeing
    request.state.user_context is set before the service injector reads it."""
    async with get_app_conversation_info_service(request.state, request) as service:
        yield service


async def _webhook_event_service(
    request: Request,
    sandbox_info: SandboxInfo = Depends(valid_sandbox),
) -> AsyncGenerator[EventService, None]:
    """Webhook-specific dependency: valid_sandbox runs first, guaranteeing
    request.state.user_context is set before the service injector reads it."""
    async with get_event_service(request.state, request) as service:
        yield service
...
async def valid_conversation(
    ...
    sandbox_info: SandboxInfo = Depends(valid_sandbox),
    app_conversation_info_service: AppConversationInfoService = Depends(_webhook_app_conversation_info_service),
) -> AppConversationInfo:
    ...
    # Sanity check - Make sure that the conversation and sandbox were created by
    # the same user.  Skip this check when the sandbox has no user_id (legacy
    # ProcessInfo records where user_id was not persisted at sandbox start time).
    if (
...
async def on_event(
    ...
    app_conversation_info_service: AppConversationInfoService = Depends(_webhook_app_conversation_info_service),
    event_service: EventService = Depends(_webhook_event_service),
) -> Success:
    """Webhook callback for when event stream events occur."""
    try:
        # Save events sequentially — asyncpg does not allow concurrent operations
        # on the same db session (which is shared within a request).
        for event in events:
