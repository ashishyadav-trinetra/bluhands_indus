...
class SetTitleCallbackProcessor(EventCallbackProcessor):
    ...
    async def __call__(
        ...
        async with (
            ...
            # The backend talks to the agent-server directly on localhost to
            # avoid the self-signed reverse-proxy certificate; the browser still
            # uses the public proxied URL.
            app_conversation_url = replace_localhost_hostname_for_docker(
                to_internal_agent_server_url(app_conversation_url)
            )
