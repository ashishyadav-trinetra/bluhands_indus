...
def config_from_env() -> AppServerConfig:
    ...
    if config.event is None:
        # Prefer local Postgres when DB_HOST is configured (dev + production with
        # managed Postgres).  Fall back to Supabase REST client only when there is
        # no local DB configured — this avoids a round-trip to the Supabase REST
        # API for every event write when the data lives in the same Postgres anyway.
        if os.getenv('DB_HOST'):
            from openhands.app_server.event.sql_event_service import (
                SqlEventServiceInjector,
            )

            config.event = SqlEventServiceInjector()
        elif os.getenv('SUPABASE_URL'):
            from openhands.app_server.event.supabase_event_service import (
                SupabaseEventServiceInjector,
            )

            config.event = SupabaseEventServiceInjector()
        else:
    ...
    if config.sandbox is None:
        ...
        if os.getenv('RUNTIME') == 'remote':
            ...
            process_sandbox_kwargs = {}
            if os.getenv('SANDBOX_CONTAINER_URL_PATTERN'):
                process_sandbox_kwargs['container_url_pattern'] = os.environ[
