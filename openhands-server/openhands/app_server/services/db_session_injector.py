...
class DbSessionInjector(BaseModel, Injector[AsyncSession]):
    ...
    async def get_async_db_engine(self) -> AsyncEngine:
        ...
        if self.gcp_db_instance:  # GCP environments
            ...
            if self.host:
                # Build connect_args for SSL support (required by Supabase and
                ...
                # Supabase connection poolers (Supavisor / pgbouncer in
                # transaction or statement pooling mode) can route different
                # queries on the same logical connection to different backend
                # connections. asyncpg's prepared-statement caching assumes a
                # stable backend per connection, so under pooling this causes
                # asyncpg.exceptions.DuplicatePreparedStatementError /
                # InvalidSQLStatementNameError. Disable prepared statement
                # caching so this works regardless of pooler mode.
    ...
    def get_db_engine(self) -> Engine:
        ...
        if self.gcp_db_instance:  # GCP environments
            ...
            # Build connect_args for SSL support (sync engine uses pg8000)
            sync_connect_args: dict = {}
            if self.host and os.getenv('DB_SSL', 'true').lower() != 'false':
                import ssl as _ssl
                ssl_ctx = _ssl.create_default_context()
                ssl_ctx.check_hostname = False
                ssl_ctx.verify_mode = _ssl.CERT_NONE
                sync_connect_args['ssl_context'] = ssl_ctx
            ...
            engine = create_engine(
                ...
                pool_recycle=self.pool_recycle,
                pool_pre_ping=True,
                connect_args=sync_connect_args if self.host else {},
            )
